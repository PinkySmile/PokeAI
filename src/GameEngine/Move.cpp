...
namespace PokemonGen1
{
		...
		_missCallBackDescription(missCallBackDescription)
	{
		...
		if (this->_statusChange.prob != 0) {
			if (this->_statusChange.prob >= 1)
				stream << "Makes foe ";
			else
				stream << (this->_statusChange.prob * 100) << "% chance to make foe ";
		...
		for (auto &change : this->_ownerChange) {
			if (change.prob >= 1)
				...
				stream << "Has " << (change.prob * 100) << "% chance to " << (change.nb > 0 ? "increase" : "decrease");
	...
	bool Move::attack(Pokemon &owner, Pokemon &target, const std::function<void(const std::string &msg)> &logger)
	{
		...
		if (this->_power)
			target.takeDamage(damages.damages * hits);
	...
	*/
	...
	const std::vector<Move> availableMoves{
		...
		Move{0x1C, "Sand-Attack" , TYPE_NORMAL  , STATUS  ,   0, 100, 15, NO_STATUS_CHANGE, {}, {{STATS_ACC, -1, 1}}},
		...
		Move{0x6C, "SmokeScreen" , TYPE_NORMAL  , STATUS  ,   0, 100, 20, NO_STATUS_CHANGE, {}, {{STATS_ACC, -1, 1}}},
		...
		Move{0x7B, "Smog"        , TYPE_POISON  , PHYSICAL,  20,  70, 20, {STATUS_POISONED, 0.4}},
		Move{0x7C, "Sludge"      , TYPE_POISON  , PHYSICAL,  65, 100, 20, {STATUS_POISONED, 0.3}},
		Move{0x7D, "Bone Club"   , TYPE_GROUND  , PHYSICAL,  65,  85, 20},
		Move{0x7E, "Fire Blast"  , TYPE_FIRE    , SPECIAL , 120,  85,  5, {STATUS_BURNED, 0.1}},
		Move{0x7F, "Waterfall"   , TYPE_WATER   , SPECIAL ,  80, 120, 15},
		...
		Move{0x86, "Kinesis"     , TYPE_PSYCHIC , STATUS  ,   0,  80, 15, NO_STATUS_CHANGE, {}, {{STATS_ACC, -1, 1}}},
		...
		Move{0x94, "Flash"       , TYPE_NORMAL  , STATUS  ,   0,  70, 20, NO_STATUS_CHANGE, {}, {{STATS_ACC, -1, 1}}},
