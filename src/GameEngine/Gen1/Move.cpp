...
namespace PokemonGen1
{
		...
		wasReplaced(false)
	{
		...
		if (this->_skipAccuracyCheck)
			stream << "Skips accuracy check" << std::endl;
	...
	bool Move::attack(Pokemon &owner, Pokemon &target, const BattleLogger &logger)
	{
		...
		if (!this->_nbHit) {
			...
			if (this->_nbRuns.second == this->_nbRuns.first)
				...
				this->_nbHit = this->_nbRuns.first + (rng() & 1U);
	...
	skipAccuracyAndDamageCheck:
		...
		if (this->_nbHits.second == this->_nbHits.first)
			...
			hits = this->_nbHits.first + (rng() & 1U);
	...
	*/
	...
	const std::array<Move, 256> availableMoves{
		Move{0x00, "-"           , TYPE_INVALID , STATUS,    0,    0,  0, NO_STATUS_CHANGE, NO_STATS_CHANGE, DEFAULT_HITS, ONE_RUN, 0, DEFAULT_CRIT_CHANCE, NO_LOADING, false, false, nullptr, nullptr, "No move. This move is invalid and will cause desync when used."},
