...
import time
import random
from HeuristicAI import make_ai, make_competitive_ai
...
ai_heur = make_competitive_ai()
rng = default_rng(int(time.time()))
emulator = PyBoyEmulator(rom='/home/pinky/pokeyellow/pokeyellow.gbc')
battle = BattleHandler(False, False)
state = battle.state
state.rng.generate_list(9)
state.logger = print
scenario_file = scenario_folder + "/" + random.choice(os.listdir(scenario_folder))
print(scenario_file)
with open(scenario_file, "rb") as fd:
	data = fd.read()
	state.me.name, state.me.team = load_trainer(data[:TRAINER_DATA_SIZE], state, False)
	state.op.name, state.op.team = load_trainer(data[TRAINER_DATA_SIZE:], state, True)
