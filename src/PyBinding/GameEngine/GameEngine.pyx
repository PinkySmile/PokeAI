...
def convertString(data):
	...
	if isinstance(data, str):
		s = data.encode('ASCII')
		return EmulatorGameHandle.convertString(s)
	d = data
	return EmulatorGameHandle.convertString(d)
...
cdef class BattleHandler:
	...
	def saveState(self, path):
		if isinstance(path, str):
			path = path.encode('ASCII')
		return self.__instance.saveState(path)

	def loadState(self, path):
		if isinstance(path, str):
			path = path.encode('ASCII')
		return self.__instance.loadState(path)
