...
#include <stdio.h>
#include <string.h>
#include "GameEngine/Pokemon.h"
#include "GameEngine/BattleHandler.h"
#include "Emulator/BgbHandler.h"
#include "GameEngine/State.h"
#include "GameEngine/RandomGenerator.h"
...
enum PokemonGen1_BattleAction getAction(const char *val)
{
	struct ActionPair {
		const char *name;
		PokemonGen1_BattleAction action;
	};

	const static struct ActionPair actions[] = {
		{ "Attack1", PokemonGen1_Attack1 },
		{ "Attack2", PokemonGen1_Attack2 },
		{ "Attack3", PokemonGen1_Attack3 },
		{ "Attack4", PokemonGen1_Attack4 },
		{ "Switch1", PokemonGen1_Switch1 },
		{ "Switch2", PokemonGen1_Switch2 },
		{ "Switch3", PokemonGen1_Switch3 },
		{ "Switch4", PokemonGen1_Switch4 },
		{ "Switch5", PokemonGen1_Switch5 },
		{ "Switch6", PokemonGen1_Switch6 },
		{ "Struggle",PokemonGen1_StruggleMove },
		{ "Run",     PokemonGen1_Run }
	};
...
{
#define PRINT_ERROR(fmt, ...) do { printf(fmt"\n", ##__VA_ARGS__); goto cleanup; } while (0)
	char *line = NULL;
	size_t size = 0;
	unsigned id = 256;
	...
	char *tmp;
	PokemonGen1_Pokemon *result = NULL;

	if (!space)
		PRINT_ERROR("Missing specie ID/name");

	*space = 0;
	id = strtol(space, &tmp, 0);
	if (*tmp) {
		size_t out = 0;

		for (size_t in = 0; line[in] != ' '; in++)
			if (isalnum(line[in]))
				line[out++] = toupper(line[in]);
		line[out] = '\0';
		for (size_t i = 0; i < MAX_POKEMON_ID; i++) {
			PokemonGen1_Pokemon_Base *pkmn = PokemonGen1_getPokemonBase(i);
			char *name = malloc(strlen(pkmn->name));

			for (size_t j = 0, k = 0; name[j]; j++)
				if (isalnum(pkmn->name[j]))
					name[k++] = pkmn->name[j];
			PokemonGen1_Pokemon_Base_destroy(pkmn);
	...
	if (id > 255)
		PRINT_ERROR("Invalid pokemon specie. Type `pokemons` for a list of available species.");

	space = strchr(space + 1, ' ');
	if (!space)
		PRINT_ERROR("Missing level");
	*space = 0;
	unsigned char level = strtol(space, &tmp, 0);
	if (*tmp)
		PRINT_ERROR("Specified level is not a valid number");
